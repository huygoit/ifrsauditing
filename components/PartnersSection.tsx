...
function LogoTile({ logo, decorative, grid }: { logo: PartnerLogo; decorative?: boolean; grid?: boolean }) {
  const inner = (
    <div
      className={[
        "group flex h-24 items-center justify-center rounded-2xl border border-slate-200/80 bg-white px-7 shadow-[0_2px_16px_rgba(15,23,42,0.04)] transition duration-300 hover:-translate-y-1 hover:border-emerald-300 hover:shadow-brand md:h-28 md:px-8",
        grid ? "w-full" : "w-56"
      ].join(" ")}
    >
...
export function PartnersSection({ logos }: { logos: PartnerLogo[] }) {
  const t = useTranslations("ifrs.partners");

  // Không có dữ liệu thì ẩn khu vực để trang chủ không bị trống
  ...
  return (
    <section className="border-t border-slate-200/80 bg-slate-50 py-10 md:py-12">
      <div className="mx-auto max-w-[1200px] px-4 sm:px-6 lg:px-8">
        ...
        <ul className="mt-10 grid grid-cols-2 gap-3 sm:grid-cols-3 md:hidden">
          {logos.map((logo) => (
            <LogoTile key={`grid-${logo.id}`} logo={logo} grid />
          ))}
        </ul>
        ...
        <div className="marquee-wrap relative mt-12 hidden overflow-hidden rounded-3xl border border-slate-200/80 bg-white p-8 shadow-[0_8px_40px_rgba(15,23,42,0.06)] md:block">
          ...
          <div className="relative space-y-5">
            <ul className="animate-marquee flex w-max items-center gap-5">
              {rowTop.map((logo, i) => (
                <LogoTile key={`top-${i}`} logo={logo} decorative={i >= logos.length} />
              ))}
            </ul>
            <ul className="animate-marquee-rev flex w-max items-center gap-5">
              {rowBottom.map((logo, i) => (
                <LogoTile key={`bottom-${i}`} logo={logo} decorative={i >= logos.length} />
              ))}
            </ul>
          </div>
