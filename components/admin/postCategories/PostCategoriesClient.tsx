...
export function PostCategoriesClient({ initialLang }: { initialLang: Lang }) {
  ...
  async function openEdit(id: string) {
    ...
    if (lang === "en") {
      const res = await fetch(`/api/admin/post-categories/${id}`, { cache: "no-store" });
      if (res.ok) {
        const json = await res.json();
  ...
  async function save() {
    ...
    try {
      ...
      const url = mode === "create" ? "/api/admin/post-categories" : `/api/admin/post-categories/${editing?.id}`;
  ...
  async function toggleStatus(id: string, next: Row["status"]) {
    ...
    const res = await fetch(`/api/admin/post-categories/${id}`, {
      method: "PATCH",
  ...
  async function bulk(action: "set_status_active" | "set_status_inactive") {
    ...
    await fetch("/api/admin/post-categories/bulk", {
      method: "POST",
      headers: { "content-type": "application/json" },
  ...
  async function move(id: string, dir: "up" | "down") {
    ...
    next[j] = tmp;
    setItems(next);
    await fetch("/api/admin/post-categories/reorder", {
      method: "POST",
      headers: { "content-type": "application/json" },
    ...
    <div className="grid gap-6">
      <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <h1 className="text-2xl font-semibold tracking-tight text-slate-900">{tAdmin(lang as any, "admin.post_categories.title")}</h1>
            <p className="mt-2 max-w-2xl text-sm leading-relaxed text-slate-600">
              {tAdmin(lang as any, "admin.post_categories.subtitle")} <span className="font-semibold">?lang</span>.
            </p>
          </div>
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-[1fr_180px] sm:items-end">
          <label className="grid gap-2">
            ...
            <input
              ...
              placeholder={tAdmin(lang as any, "admin.post_categories.search_placeholder")}
            />
          </label>
          <label className="grid gap-2">
        ...
        {selectedIds.length ? (
          <div className="mb-3 flex flex-wrap items-center justify-between gap-2 rounded-2xl border border-slate-200 bg-slate-50 px-3 py-2">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              >
                {tAdmin(lang as any, "admin.post_categories.bulk_set_active")}
              </button>
              <button
                type="button"
              ...
              >
                {tAdmin(lang as any, "admin.post_categories.bulk_set_inactive")}
              </button>
              <button
                type="button"
                onClick={() => setSelected({})}
        ...
        <div className="grid gap-3">
          {loading ? (
              ...
              return (
                ...
                >
                  <input
                    ...
                    aria-label={`${tAdmin(lang as any, "admin.post_categories.select")} ${row.translation.name || row.id}`}
                  />
            ...
            <div className="rounded-2xl border border-slate-200 bg-slate-50 p-6 text-sm text-slate-600">
              {tAdmin(lang as any, "admin.post_categories.empty")}
            </div>
          )}
        </div>
      </div>
      ...
      <QuickEditDrawer
        ...
        title={
          mode === "create"
            ? tAdmin(lang as any, "admin.post_categories.create_title")
            : editing?.translation?.name
              ? `${tAdmin(lang as any, "common.edit")}: ${editing.translation.name}`
              : tAdmin(lang as any, "admin.post_categories.edit_title")
        }
      ...
      >
        ...
        <div className="grid gap-4">
          <Field label={tAdmin(lang as any, "admin.post_categories.form.name")}>
            <input
              value={name}
              onChange={(e) => {
          ...
          <Field label={tAdmin(lang as any, "admin.post_categories.form.slug")} hint={tAdmin(lang as any, "admin.post_categories.form.slug_hint")}>
            <input
              value={slug}
              onChange={(e) => setSlug(slugifyAscii(e.target.value))}
              className="w-full rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm shadow-sm transition focus-visible:ring-2 focus-visible:ring-emerald-500 focus-visible:ring-offset-2"
              placeholder={tAdmin(lang as any, "admin.post_categories.form.slug_placeholder")}
            />
          </Field>
          <Field label={tAdmin(lang as any, "admin.post_categories.form.description")}>
            <textarea
              value={description}
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <Field label={tAdmin(lang as any, "admin.post_categories.form.seo_title")}>
              <input
                value={seoTitle}
            ...
            <Field label={tAdmin(lang as any, "admin.post_categories.form.seo_desc")}>
              <textarea
                value={seoDesc}
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <Field label={tAdmin(lang as any, "admin.post_categories.form.sort_order")}>
              <input
                type="number"
                value={sortOrder}
