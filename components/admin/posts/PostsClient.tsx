...
export function PostsClient({ initialLang }: { initialLang: Lang }) {
  ...
  const [postCategories, setPostCategories] = useState<Array<{ id: string; name: string; slug: string }>>([]);
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const res = await fetch(`/api/admin/post-categories?${sp.toString()}`, { cache: "no-store" });
  ...
  async function load() {
    ...
    try {
      ...
      const res = await fetch(`/api/admin/posts?${sp.toString()}`, { cache: "no-store" });
  ...
  async function openEdit(id: string) {
    ...
    setPostCategoryId(String((row as any)?.postCategoryId ?? ""));
    setContentMarkdown("");
    setContentJson(null);
    setSeoTitle("");
    setSeoDesc("");
    ...
    if (res.ok) {
      const json = await res.json();
      setPostCategoryId(String((json as any)?.item?.postCategoryId ?? ""));
  ...
  async function save() {
    ...
    try {
      ...
      const payload = {
        ...
        postCategoryId: postCategoryId.trim() ? postCategoryId.trim() : null,
      ...
      const url = mode === "create" ? "/api/admin/posts" : `/api/admin/posts/${editing?.id}`;
  ...
  async function bulk(action: "set_status_draft" | "set_status_published" | "set_status_scheduled") {
    ...
    await fetch("/api/admin/posts/bulk", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ ids, action }) });
    await load();
  }
  ...
  async function move(id: string, dir: "up" | "down") {
    ...
    next[j] = tmp;
    setItems(next);
    await fetch("/api/admin/posts/reorder", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ ids: next.map((x) => x.id) }) });
    await load();
  }
    ...
    <div className="grid gap-6">
      <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <h1 className="text-2xl font-semibold tracking-tight text-slate-900">{tAdmin(lang as any, "admin.posts.title")}</h1>
            <p className="mt-2 max-w-3xl text-sm leading-relaxed text-slate-600">
              {tAdmin(lang as any, "admin.posts.subtitle")} <span className="font-semibold">?lang</span>.
            </p>
          </div>
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-4 md:items-end">
          <label className="grid gap-2 md:col-span-3">
            ...
            <input value={q} onChange={(e) => setQ(e.target.value)} className="w-full rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm shadow-sm transition focus-visible:ring-2 focus-visible:ring-emerald-500 focus-visible:ring-offset-2" placeholder={tAdmin(lang as any, "admin.posts.search_placeholder")} />
          </label>
          <label className="grid gap-2">
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
        ...
        {loading ? (
          ...
          <div className="rounded-2xl border border-slate-200 bg-slate-50 p-6 text-sm text-slate-600">{tAdmin(lang as any, "admin.posts.empty")}</div>
        )}
      </div>
      ...
      <QuickEditDrawer
        ...
        title={
          mode === "create"
            ? tAdmin(lang as any, "admin.posts.drawer.create")
            : editing?.translation?.title
              ? `${tAdmin(lang as any, "admin.posts.drawer.edit_prefix")} ${editing.translation.title}`
              : tAdmin(lang as any, "admin.posts.drawer.edit_prefix")
        }
      ...
      >
        ...
        <div className="grid gap-4">
          ...
          <Field label={tAdmin(lang as any, "admin.posts.form.category")}>
            <select
              value={postCategoryId}
              onChange={(e) => setPostCategoryId(e.target.value)}
              className="w-full rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm shadow-sm transition focus-visible:ring-2 focus-visible:ring-emerald-500 focus-visible:ring-offset-2"
            >
              <option value="">{tAdmin(lang as any, "admin.posts.form.category_none")}</option>
          ...
          <Field label={tAdmin(lang as any, "admin.posts.form.cover_image")}>
            <FileUploader folder="uploads/posts" value={coverImage} onChange={setCoverImage} accept="image/*" />
          </Field>
