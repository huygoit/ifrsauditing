...
function formatIfrsDate(locale: NewsLocale, d: Date | null) {
  if (!d) return "";
...
export default async function IfrsPage({ params }: { params: { locale: NewsLocale } }) {
  ...
  const items = await getContentsByType(locale, { type: "IFRS", limit: 9 });
        ...
        <section id="bai-viet" className="scroll-mt-20 border-t border-slate-200/80 bg-slate-50 py-16 md:py-24">
          <div className="mx-auto max-w-[1200px] px-4 sm:px-6 lg:px-8">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
              ...
              <a href={`/${locale}/noi-dung/ifrs`} className="group hidden shrink-0 items-center gap-1.5 text-sm font-semibold text-emerald-700 hover:text-emerald-800 sm:inline-flex">
                {t("articles.viewAll")}
            ...
            {items.length ? (
              <div className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                {items.map((item) => {
                  const href = `/${locale}/noi-dung/${item.category?.slug ?? "ifrs"}/${item.slug}`;
                  const dateText = formatIfrsDate(locale, item.publishedAt ?? item.updatedAt);
                  return (
                    <a
                      key={item.id}
                      href={href}
                      className="group flex h-full flex-col overflow-hidden rounded-2xl border border-slate-200/90 bg-white shadow-[0_2px_20px_rgba(15,23,42,0.05)] transition duration-300 hover:-translate-y-1 hover:border-emerald-300 hover:shadow-brand focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500 focus-visible:ring-offset-2"
                    >
