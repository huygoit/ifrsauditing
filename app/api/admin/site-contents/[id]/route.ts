...
export async function GET(req: NextRequest, ctx: { params: { id: string } }) {
  ...
  const jsonRows = (await prisma.$queryRaw`
    SELECT siteContentId, lang, contentJson
...
export async function PATCH(req: NextRequest, ctx: { params: { id: string } }) {
  ...
  try {
    await prisma.$transaction(async (tx) => {
      ...
      if (siteContentCategoryId !== undefined) {
        await tx.$executeRaw`
          UPDATE sitecontent
      ...
      if (
        ...
        const t = await tx.siteContentTranslation.findUnique({
          where: { siteContentId_lang: { siteContentId: id, lang } }
        });
        if (t) {
          await tx.siteContentTranslation.update({
            where: { id: t.id },
            data: {
          ...
          if (contentJson !== undefined) {
            await tx.$executeRaw`UPDATE sitecontenttranslation SET contentJson = ${contentJson} WHERE id = ${t.id}`;
          }
        } else {
          const created = await tx.siteContentTranslation.create({
            data: {
              siteContentId: id,
              lang,
          ...
          if (contentJson !== undefined) {
            await tx.$executeRaw`UPDATE sitecontenttranslation SET contentJson = ${contentJson} WHERE id = ${created.id}`;
          }
        }
      }
    });
    ...
    console.error("[admin/site-contents/[id]][PATCH] update failed", e);
