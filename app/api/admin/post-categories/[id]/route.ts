...
export async function GET(req: NextRequest, ctx: { params: { id: string } }) {
  ...
  const translations = (await prisma.$queryRaw`
    SELECT id, postCategoryId, lang, name, slug, description, seoTitle, seoDesc
    FROM postcategorytranslation
    WHERE postCategoryId = ${id}
  `) as Array<{ id: string; lang: "vi" | "en"; name: string; slug: string; description: string | null; seoTitle: string | null; seoDesc: string | null }>;
...
export async function PATCH(req: NextRequest, ctx: { params: { id: string } }) {
  ...
  try {
    await prisma.$transaction(async (tx) => {
      ...
      if (name !== undefined || description !== undefined || normalizedSlug !== undefined || seoTitle !== undefined || seoDesc !== undefined) {
        const tRows = (await tx.$queryRaw`
          ...
          FROM postcategorytranslation
          WHERE postCategoryId = ${id} AND lang = ${lang}
          LIMIT 1
        ...
        if (t) {
          await tx.$executeRaw`
            UPDATE postcategorytranslation
    ...
    console.error("[admin/post-categories/[id]][PATCH] update failed", e);
